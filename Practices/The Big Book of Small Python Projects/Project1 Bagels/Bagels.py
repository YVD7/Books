...
def main():
    ...
    while True: # Main game loop.
        ...
        while numGuesses <= MAX_GUSSES:
            ...
            if guess == secretNum:
              break # They're correct, to break out of this loop
...
def getSecretNum():
    """Return a sting made up of NUM_DIGITS unique random digits."""
    numbers = list('0123456789') # create a list of digits 0 to 9
    random.shuffle(numbers) # shuffle them into random order.

    # Get the first NUM_DIGITS digits in the list for the secrete number:
    secretNum = ''
...
def getClues(guess, secretNum):
    ...
    if len(clues) == 0:
        ...
        clues.sort()
        # Make a single string from the list of string
        return ' '.join(clues)

# If the program is run (instead of imported), run game:
